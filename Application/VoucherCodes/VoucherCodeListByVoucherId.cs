...
namespace Application.VoucherCodes
{
    ...
    public class VoucherCodeListByVoucherId
    {
        public class Query : IRequest<Result<List<VoucherCodeDto>>>
        {
            public Query(int page, int limit, int voucherId)
            {
                Page = page;
                Limit = limit;
                VoucherId = voucherId;
        ...
        public class Handler : IRequestHandler<Query, Result<List<VoucherCodeDto>>>
        {
            ...
            public async Task<Result<List<VoucherCodeDto>>> Handle(Query request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    var totalRecord = await _context.VoucherCode.Where(vc => vc.VoucherId == request.VoucherId)
                    ...
                    if (request.Page <= lastPage)
                        voucherCodes = await _context.VoucherCode.Where(vc => vc.VoucherId == request.VoucherId)
                            .OrderBy(v => v.VoucherCodeId)
                    ...
                    _logger.LogInformation(
                        "Successfully retrieved list of all voucher codes by voucherId " + "{VoucherId}",
                        request.VoucherId);
                    return Result<List<VoucherCodeDto>>.Success(voucherCodes,
                        "Successfully retrieved list of all voucher codes by " + $"{request.VoucherId}.",
                        paginationDto);
                }
