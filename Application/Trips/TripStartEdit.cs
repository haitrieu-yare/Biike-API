        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    Trip trip = await _context.Trip.Where(t => t.TripId == request.TripId)
                        .Include(t => t.Route)
                    ...
                    {
                        _logger.LogInformation("Trip must has Biker before waiting or starting");
                        return Result<Unit>.Failure("Trip must has Biker before waiting or starting.");
                    }
