...
namespace API.Controllers
{
	...
	public class TripsController : BaseApiController
	{
		...
		public async Task<IActionResult> EditTripProgressTime(
			int tripId, int bikerId, CancellationToken ct)
		{
			int role = ControllerUtils.GetRole(HttpContext);
			...
			return HandleResult(await Mediator.Send(
				new EditTripProcess.Command { TripId = tripId, BikerId = bikerId }, ct));
		}

		// Keer, Biker, Admin
		[HttpPut("{tripId:int}/cancel")]
		public async Task<IActionResult> EditTripCancellation(int tripId,
